...
def notify_due_digest():
    ...
    if pms:
        ...
        for pm in pms:
            ...
            item = f"• PM-{pm.id} {pm.main_equipment or ''} — {flag} ({pm.next_due}) — {_username(pm.assigned_user_id)}"
            lines.append(item)
            if pm.assigned_user_id:
                per_user[pm.assigned_user_id].append(item)
    ...
    if wos:
        if lines:
            lines.append("")
        lines.append("*Work orders due or overdue*")
        for wo in wos:
            flag = "OVERDUE" if wo.expected_completion_date < today else "DUE TODAY"
            item = f"• WO-{wo.id} {wo.equipment or ''} — {flag} ({wo.expected_completion_date}) — {_username(wo.assigned_to_id)}"
            lines.append(item)
            if wo.assigned_to_id:
                per_user[wo.assigned_to_id].append(item)
